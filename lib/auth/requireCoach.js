...
// ⚠️ DEPRECATED — do not use on new routes (security review M8).
// This answers "is this a real token", not "is this token allowed to act on
// this client". used_at is set forever, so every athlete who ever onboarded
// holds a permanent pass. It was the only gate on /api/gym-scan, giving any
// past athlete unlimited Claude Vision calls. Use tokenMatchesClient()
// instead, which binds the token to the client being acted on.
// Currently unreferenced; kept only so an older deploy doesn't break.
